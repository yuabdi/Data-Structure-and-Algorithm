...
//
// Compile this assignment with: gcc -g -Wall queue_t.c queue_test.c -o prog
//
// Run with: ./prog
//
...
// Test the size of empty queue initialization
int unitTest2(int status){
    int passed = 0;
	queue_t* test1 = create_queue(1);
    if(0==queue_size(test1)){
        passed = 1;
    }
...
// A simple enqueue operation
// Test the size
int unitTest4(int status){
    int passed = 0;
	queue_t* test1 = create_queue(1);
	queue_enqueue(test1,1);	
    if(1==queue_size(test1)){
        passed = 1;
    }
...
// Partial dequeue after enqueing
...
// Test queue_full and queue_empty operations
