...
void sortIntegers(int* array, unsigned int size){
   ...
   for (int i = 1; i < size; i++) {            // iterate from index 1 to end of list
    index = i;                                // current index
    value = array[i];                         // current value

    for (int j = index; j > 0; j-- ) {        // iterate from current value to start
        if (value < array[j - 1])              // check current value is less than the j index
          swap(&array[j], &array[j - 1]);      // swap values
    }
  }
}
